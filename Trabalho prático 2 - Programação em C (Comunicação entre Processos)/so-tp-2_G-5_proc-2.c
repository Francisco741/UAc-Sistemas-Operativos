...
int sinal_1() {
	...
	FILE *pid_programa_2 = fopen("pid_programa_2.txt", "r");
    pid_t pid_prog_2;
    fscanf(pid_programa_2, "%d", &pid_prog_2);
    fclose(pid_programa_2);
    // envia o sinal SIGUSR1 para um processo
    kill(pid_prog_2, SIGUSR1);
...
int main() {
    ...
    FILE *pid_programa_1 = fopen("pid_programa_1.txt", "w");
    // Escrever o PID no arquivo
    fprintf(pid_programa_1, "%d", getpid());
